...
if __name__ == "__main__":
    ...
    for ratio in ratio_list:
        ...
        if model_select_ver == 'only_tune':
            ...
            model_path = str(CHECKPOINT_DIR / f"fine-tune-model{fine_tune_ver}/{fine_tune_num}/gps_model.pt")
        else:
            model_path = ''

        train_embedding_pre_path = EMBEDDING_DIR / f"{model_select_ver}_ver{fine_tune_ver}/train/{fine_tune_num}"
        train_embedding_pre_path.mkdir(parents=True, exist_ok=True)
        train_embedding_path = str(train_embedding_pre_path / "train_embedding.csv")

        test_embedding_pre_path = EMBEDDING_DIR / f"{model_select_ver}_ver{fine_tune_ver}/test/{fine_tune_num}"
        test_embedding_pre_path.mkdir(parents=True, exist_ok=True)
        test_embedding_path = str(test_embedding_pre_path / "test_embedding.csv")

        if WANDB_API_KEY:
            wandb.init(
                entity=os.environ.get("WANDB_ENTITY", ""),
                project="CIC-IDS2017",
                name=f"evaluation-2class-subgraph-{model_select_ver}-ver{fine_tune_ver}-{fine_tune_num}%",
                tags=['evaluation', 'format', 'fine-tune'],
                config={
