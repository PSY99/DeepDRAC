"""Build alert graphs from Snort logs and partition via Louvain community detection.

Converts CIC-IDS2017 alert logs into alert graphs (IP nodes, alert-signature edges),
applies Louvain community detection, then refines partitions using frequent
pattern rules mined from the data.
"""

import sys
import os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
sys.path.insert(0, os.path.dirname(__file__))  # for same-directory imports
...
class Log2graph():
    ...
    """
    def __init__(self, csv_name="monday-alert-label", start_time="", end_time="", out_path=None, time_interval=60*10):
        # self.csv_path = str(ALERT_DIR / csv_name)  # non-corrected version
        self.csv_path = str(ALERT_CORRECTED_DIR / csv_name)
        self.start_time = start_time
        self.end_time = end_time
        self.time_interval = time_interval
        base_name = csv_name.replace('.csv', '')
        self.out_path = out_path if out_path else str(GRAPH_DATA_DIR / f"graph_{base_name}.csv")
    ...
    def getGraph_withLog(self, input_data_df:pd.DataFrame):
        ...
        graph=nx.Graph()
        # Use only
        for louvain_edge_key,values in louvain_edge.items():
            sip,dip = louvain_edge_key
            graph.add_edge(sip,dip,weights=values)
        partition = community.best_partition(graph)
        # Second-pass refinement
        partition_edges = graph_split(partition,merge_log)
...
def graph_split(partition, merge_log):
    """
...
if __name__=="__main__":
    ...
    csv_name = 'train.csv'
    # out_path = str(GRAPH_DATA_DIR / f'graph_{csv_name}')
    out_path = str(GRAPH_DATA_DIR / 'correct' / f'graph_{csv_name}')
    log2graph = Log2graph(csv_name=csv_name, start_time="2017-07-03 08:00:00", end_time="2017-07-07 20:00:00",out_path=out_path)
